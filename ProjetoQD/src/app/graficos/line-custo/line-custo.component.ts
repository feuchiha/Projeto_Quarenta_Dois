...
export class LineCustoComponent implements OnInit, IFilter, GraficoPrevisao {
  ...
  drawChart = () => {
    ...
    const options = {
      width: 1200,
      height: 520,
      title: 'Previsão dos custos de internações do genero '+ this.cards[0].filtro.genero  +' de ' + this.cards[0].filtro.faixaEtaria + ' em de '+ this.cards[0].filtro.mes +' a Dez 2018 os dados reais e  Jan a Jun de 2019 a previsão na ' + this.cards[0].filtro.regio,
      curveType: 'function',
      legend: {
        position: 'bottom'
      },
